...
The cell lines passing with Z Prime factor values greater than or equal to 0 are:
`r unique(DT$CellLine[DT$ZPrimeRobustLoess>=0])`

The cell lines passing with SSMD values less than or equal to -3 are:
`r unique(DT$CellLine[DT$SSMDLoess <= -3])`

The cell lines passing with SSMD values less than or equal to -2 are:
`r unique(DT$CellLine[DT$SSMDLoess <= -2])`
