...
cDT$OmeroDetailURL <- paste0('<a href="https://meplincs.ohsu.edu/webclient/img_detail/',cDT$ImageID,'/"',' target="_blank">Omero</a>')
  cDT$OmeroThumbnailURL <- paste0('<a href="https://meplincs.ohsu.edu/webclient/render_thumbnail/',cDT$ImageID,'/"',' target="_blank">Omero</a>')
  cDT$OmeroImageURL <- paste0('<a href="https://meplincs.ohsu.edu/webclient/render_image/',cDT$ImageID,'/"',' target="_blank">Omero</a>')
wDT <- fread("AtwatersWell_v1.0.txt")
wDT$OmeroDetailURL <- paste0('<a href="https://meplincs.ohsu.edu/webclient/img_detail/',wDT$ImageID,'/"',' target="_blank">Omero</a>')
  wDT$OmeroThumbnailURL <- paste0('<a href="https://meplincs.ohsu.edu/webclient/render_thumbnail/',wDT$ImageID,'/"',' target="_blank">Omero</a>')
  wDT$OmeroImageURL <- paste0('<a href="https://meplincs.ohsu.edu/webclient/render_image/',wDT$ImageID,'/"',' target="_blank">Omero</a>')
...
p <- ggplot(DT, aes(x=factor(Barcode), y=ZPrimeRobust, colour=CellLine))+
  geom_point(size = 5)+
...
```{r AURKAcontrolsAnalysis,  fig.width=8, fig.height=6, eval=FALSE}

setkey(cDT,GeneSymbol)
DT <- cDT[c("AURKA","NegCtrl")]
DT <- DT[,ZPrimeRobust := calcZPrime(.SD, method="robust",posCtrl="AURKA"), by="Barcode", .SDcols=c("GeneSymbol","WellCellCount")]
colNames <- c("Barcode", "CellLine", "ZPrimeRobust")
DT <- unique(DT[,colNames, with=FALSE])
...
wnfDT <- wDT[c("AURKA","NegCtrl")]
colNames <- grep("Omero|Image|Field",colnames(wnfDT),value=TRUE,invert=TRUE)
wnfDT <- unique(wnfDT[,colNames, with=FALSE])
wnfDT <- wnfDT[,ZPrimeOriginal := calcZPrime(.SD, method="original", posCtrl="AURKA"), by="Barcode", .SDcols=c("GeneSymbol","WellCellCount")]
wnfDT <- wnfDT[,ZPrimeRobust := calcZPrime(.SD, method="robust",posCtrl="AURKA"), by="Barcode", .SDcols=c("GeneSymbol","WellCellCount")]
wnfDT <- wnfDT[,ZPrimeFixsd := calcZPrime(.SD, method="fixsd",posCtrl="AURKA"), by="Barcode", .SDcols=c("GeneSymbol","WellCellCount")]
colNames <- c("Barcode", "CellLine", "ZPrimeRobust")
wnfDT <- unique(wnfDT[,colNames, with=FALSE])
...
```{r WCC_byCellLine, fig.width=8,fig.height=7, eval=TRUE}
...
p <- ggplot(dt, aes(x=GeneSymbol, y=WellCellCount, colour=Barcode))+geom_boxplot()+
  ...
  facet_wrap(~CellLine, ncol = 2, scales = "free_y")+
...
p <- ggplot(ccsDT, aes(x=factor(GeneSymbol), y=Proportion, fill=State))+
  ...
  theme(strip.text = element_text(size = rel(.7)), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=rel(.4)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(1)),legend.text=element_text(size = rel(.6)),legend.title=element_text(size = rel(.8)))
...
```{r Layout, echo=FALSE, fig.width=8,fig.height=3, eval=TRUE}
