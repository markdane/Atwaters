...
p <- ggplot(dt, aes(x=GeneSymbol, y=WellCellCount, colour=Barcode))+geom_boxplot()+
  ggtitle("Nuclei of Control Wells by Plate")+
...
```{r UnivariateSignals , fig.width=5,fig.height=3, eval=TRUE}

DT <- cDT[grepl("NegCtrl",cDT$GeneSymbol)]
p <- ggplot(DT, aes(x=TotalIntensityDAPI,fill=as.factor(DNA2N),colour=as.factor(DNA2N)))+
  ...
  theme(strip.text = element_text(size = rel(.8)), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),plot.title = element_text(size = rel(1)),legend.text=element_text(size = rel(.3)),legend.title=element_text(size = rel(.3)))
print(p)
...
p <- ggplot(cDT[cDT$GeneSymbol=="NegCtrl"], aes(x=TotalIntensityDAPI, y=log2(MeanIntensityAlexa647+1), colour=EduPositive))+
  geom_point(size=.1)+
